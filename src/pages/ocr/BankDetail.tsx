  ...
  return (
    <div className="container py-6">
      <Helmet>
        ...
        <meta name="description" content="View saved bank OCR extraction and attached image." />
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="aspect-[4/3] rounded-md border overflow-auto bg-muted/30 flex items-center justify-center">
              {item?.file_url ? (
                <img src={item.file_url} alt={item.filename ?? 'Bank document'} className="object-contain" />
              ) : (
            ...
            <Tabs defaultValue="structured">
              ...
              <TabsContent value="structured" className="space-y-3 mt-4">
                <div className="grid grid-cols-2 gap-3">
                  <Input placeholder="Reference" value={item?.data?.reference ?? ''} readOnly />
                  <Input placeholder="Date" value={item?.data?.date ?? ''} readOnly />
                  <Input placeholder="Bank" value={item?.data?.bank ?? ''} readOnly />
                  <Input placeholder="Amount" value={item?.data?.amount ?? ''} readOnly />
                </div>
                <Textarea placeholder="Memo or Notes" value={item?.data?.memo ?? ''} readOnly />
              </TabsContent>
